...
namespace TGC.MonoGame.TP.Collisions
{
    ...
    public class OrientedBoundingBox
    {
        ...
        public static OrientedBoundingBox CreateFromPoints(Vector3[] points)
        {
            if (points == null || points.Length == 0)
                throw new ArgumentException("El array de puntos no puede estar vacío.");

            // Encontrar el AABB (Axis-Aligned Bounding Box) de los puntos
            ...
            // Calcular el centro y las extensiones de la OBB
            Vector3 center = (min + max) / 2f;
            Vector3 extents = (max - min) / 2f;

            // Devolver una caja orientada con la orientación identidad (sin rotación por defecto)
            return new OrientedBoundingBox(center, extents);
        }
